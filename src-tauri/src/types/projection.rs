...
pub(crate) struct PatchRowsProjection<'a> {
    pub(crate) rows: &'a [Vec<CellValue>],
    pub(crate) formats: &'a [Vec<Option<CellFormatProjection>>],
}
...
pub(crate) struct PatchColumnProjection<'a> {
    pub(crate) values: &'a [CellValue],
    pub(crate) formats: &'a [Option<CellFormatProjection>],
}
...
impl Serialize for PatchColumnProjection<'_> {
        ...
        S: serde::Serializer,
    {
        ...
        for (row_index, cell) in self.values.iter().enumerate() {
            values.serialize_element(&CellValueProjection::new(
                cell,
                self.formats.get(row_index).cloned().flatten(),
            ))?;
        }
        values.end()
    }
