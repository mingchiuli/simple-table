...
impl SearchIndexAdapter {
    pub fn new(source: Arc<dyn SearchDocumentSourcePort>) -> Self {
        Self::from_scheduler(create_index_scheduler(source))
    }
    ...
    pub fn rebuild_sheet_index(&self, document_id: u64, sheet_index: usize) {
        let Some(snapshot) = self
            .scheduler
            .source
            .document_snapshot(document_id, None)
...
impl SearchIndexMaintenancePort for SearchIndexAdapter {
    ...
    fn schedule_work(&self, document_id: u64, source_revision: u64, work: SearchIndexWork) {
        SearchIndexAdapter::schedule_work(self, document_id, source_revision, work);
    }
...
fn create_index_scheduler(source: Arc<dyn SearchDocumentSourcePort>) -> Arc<IndexScheduler> {
...
mod tests {
    ...
    #[test]
    fn dropping_the_last_adapter_stops_its_scheduler() {
        let context = context_for_rows(vec![vec![s("value")]]);
        let scheduler = Arc::clone(&context.search.scheduler);
        let clone = context.search.clone();
