...
use crate::application::search_service::{SearchIndexPort, SearchScanLease};
...
pub struct SearchIndexAdapter {
    ...
    _lifecycle: Arc<SearchIndexAdapterLifecycle>,
}

struct SearchIndexAdapterLifecycle {
    scheduler: Arc<IndexScheduler>,
}

impl Drop for SearchIndexAdapterLifecycle {
    fn drop(&mut self) {
...
impl SearchIndexAdapter {
    ...
    fn from_scheduler(scheduler: Arc<IndexScheduler>) -> Self {
        Self {
            _lifecycle: Arc::new(SearchIndexAdapterLifecycle {
...
impl SearchScanLease for SearchScanReservation {}

impl SearchIndexPort for SearchIndexAdapter {
    fn reserve_scan_work(&self) -> Result<Box<dyn SearchScanLease>, crate::error::AppError> {
        SearchIndexAdapter::reserve_scan_work(self)
            .map(|reservation| Box::new(reservation) as Box<dyn SearchScanLease>)
    }

    fn rebuild_all_sheets_index(&self, registry: &ActiveDocumentRepository, document_id: u64) {
        SearchIndexAdapter::rebuild_all_sheets_index(self, registry, document_id);
    }
    ...
    fn rebuild_sheet_index(
        ...
        SearchIndexAdapter::rebuild_sheet_index(self, registry, document_id, sheet_index);
    }
    ...
    fn schedule_work(
        ...
        SearchIndexAdapter::schedule_work(self, document_id, work, registry);
    }

    fn cancel_document_jobs(&self, document_id: u64) {
        SearchIndexAdapter::cancel_document_jobs(self, document_id);
    }
}
...
impl Default for SearchIndexAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[allow(dead_code)]
pub fn search_scheduler_stats(search: &SearchIndexAdapter) -> SearchSchedulerStats {
    search.stats()
}
...
mod tests {
    ...
    fn isolated_search_service() -> SearchIndexAdapter {
        SearchIndexAdapter::from_scheduler(Arc::new(IndexScheduler {
            state: Mutex::new(IndexSchedulerState::default()),
            wake: Condvar::new(),
    ...
    fn isolated_search_service_without_workers() -> SearchIndexAdapter {
        SearchIndexAdapter::from_scheduler(Arc::new(IndexScheduler {
            state: Mutex::new(IndexSchedulerState::default()),
            wake: Condvar::new(),
            workers_available: AtomicBool::new(false),
